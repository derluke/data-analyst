...
class CustomJsonFormatter(logging.Formatter):
    def format(self, record):
        if hasattr(record, 'json_data'):
            try:
                if isinstance(record.json_data, dict) and 'messages' in record.json_data:
                    formatted_messages = []
                    for msg in record.json_data['messages']:
                        formatted_msg = {
                            'role': msg['role'],
                            'content': msg['content'].replace('\n', '\\n')[:100] + '...' if len(msg['content']) > 100 else msg['content']
                        }
...
def format_json(obj: Any) -> str:
    try:
        ...
        if isinstance(obj, dict) and 'messages' in obj:
            ...
            return json.dumps(formatted_obj, indent=2, sort_keys=True, default=str, ensure_ascii=False)
...
def log_api_call(func):
    async def wrapper(*args, **kwargs):
        ...
        try:
            formatted_args = [
                arg.dict() if hasattr(arg, 'dict') else arg 
                for arg in args
            ]
            formatted_kwargs = {
...
async def process_chat_and_analysis(question: str, chat_messages: list) -> None:
    error_context = {
        "question": question,
        "chat_history_length": len(chat_messages)
    }
    ...
    try:
        ...
        with st.chat_message("assistant", avatar="bot.jpg"):
            ...
            with st.spinner("Running analysis..."):
                try:
                    analysis_request = RunAnalysisRequest(
                        data={name: df['data'].to_dict('records') for name, df in st.session_state.cleansed_data.items()},
            ...
            if analysis_result and "data" in analysis_result:
                try:
                    chart_df = pd.DataFrame(analysis_result["data"]) if isinstance(analysis_result["data"], list) else pd.DataFrame([analysis_result["data"]])
                    ...
                    with st.spinner("Generating analysis..."):
                        ...
                        for coro in asyncio.as_completed(tasks):
                            try:
                                ...
                                if isinstance(result, dict) and ("fig1" in result or "fig2" in result):
                                    ...
                                    with bottom_line_container:
                                        with st.expander("Bottom Line", expanded=True):
                                            st.markdown(result.get('bottom_line', '').replace('$', '\\$'))
                                    
                                    with insights_container:
                                        if result.get("additional_insights"):
                                            with st.expander("Additional Insights", expanded=True):
                                                st.markdown(result["additional_insights"].replace('$', '\\$'))
                                ...
                                task_type = "charts" if charts_task in tasks else "business"
                                ...
                                if task_type == "charts":
                                    ...
                                    with bottom_line_container:
                                        st.error(f"Error generating business analysis: {str(e)}")
        ...
        with st.chat_message(message["role"], avatar=avatar):
            ...
            if "components" in message:
                ...
                business_analysis = next(
                    (comp for comp in message["components"] 
                ...
                with st.expander("Bottom Line", expanded=True):
                    ...
                    st.markdown(bottom_line.replace('$', '\\$') if bottom_line else "No bottom line available")
                ...
                with st.expander("Charts", expanded=True):
                    ...
                    if "fig1" in charts_component["data"]:
                        st.plotly_chart(charts_component["data"]["fig1"], use_container_width=True)
                    if "fig2" in charts_component["data"]:
                        st.plotly_chart(charts_component["data"]["fig2"], use_container_width=True)
                ...
                with st.expander("Additional Insights", expanded=True):
                    ...
                    st.markdown(insights.replace('$', '\\$') if insights else "No additional insights available")
